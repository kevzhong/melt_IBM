...
real    :: f1(3), f2(3), t1(3), t2(3), pos1(3), pos2(3), r_p(3)
...
!      call closest_distance(i,j,p1,p2,x1,x2,dist,AA1,AA2)
        ...
        dist1=norm2(pos1-pos2)
        dist1=dist1-2*rad_p
      
         if (dist1 .lt. dc) then
        !  if(ismaster)then
        !  namfile='flowmov/dist.txt'
        !  open(unit=92,file=namfile, Access='append', Status='unknown')
        !  write(92,'(2E15.7,2I4.1,1000E15.7)') time,dt,i,j,dist1,pos1,pos2
        !  close(92)
        ! end if        
        ...
        dij = p2 - p1
         if(norm2(dij).lt.2.)then
          f1 = -1e4 * ( ((dist1-dc)/dc)**2 ) * dij/norm2(dij)
          else
          f1 = 1e4 * ( ((dist1-dc)/dc)**2 ) * dij/norm2(dij)
          end if
          f2 = -f1
        call cross(t1, r_p, f1)
        call cross(t2, r_p, f2)

        t1 = matmul(AA1, t1)
        t2 = matmul(AA2, t2)

      if (coll_check.eqv. .false.) then
        fpc(1:3,i) = fpc(1:3,i) + f1
        fpc(1:3,j) = fpc(1:3,j) + f2
        tp(1:3,i) = tp(1:3,i) + t1
        tp(1:3,j) = tp(1:3,j) + t2
      endif
  !      if(ismaster)then !.and.(i.eq.2).and.(j.eq.10)) then
  !      namfile='flowmov/coll_force.txt'
      
  !      open(unit=92,file=namfile, Access='append', Status='unknown')
  !      write(92,'(2E15.7,2I4.1,1000E15.7)') time,dt,i,j,fp(1:3,i),fp(1:3,j),r_p,tp(1:3,i),tp(1:3,j)
  !      close(92)
  !      end if
      endif
...
pos1 = pos_cm(1:3, i) + matmul(AAT1, y1)
pos2 = closest_particle(pos_cm(1:3, i), pos_cm(1:3, j))

pos2 = pos2 + matmul(AAT2,y1)
!do np = 1,3
