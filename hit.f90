subroutine CalcHITRandomForce
    ! Compute HIT forcing using the Stochastic wavenumber forcing scheme of Eswaran & Pope (1988)
    ! Forcing in wavenumber space is computed in physical space through explicit evaluation of the DFT summations
    ! cf. Appendix A of Chouippe & Uhlmann (2015, Phys. Fluids)
      use param
      use mpih
...
subroutine InitRandomForce
    ! Initialise terms to be re-used for computation in the Stochastic wavenumber forcing scheme of Eswaran & Pope (1988)
    use param
    use mpih
    use mpi_param, only: kstart, kend
    implicit none
...
subroutine CalcABC_HITForce
    ! Compute RHS "HIT" forcing using the spatially- & temporally-constant ABC forcing scheme
    ! E.g.  Podvigina & Pouquet (1994, Physica D)
    !       Mininni et al. (2006, Phys. Rev. E) 
    !       Chiarini & Rosti (2024, JFM)
      use param
      use local_arrays, only: forcx,forcy,forcz
      use mpih
      use mpi_param
  ...
  subroutine WriteRandForcCoef
    ...
    if(ismaster) then 
        ...
        call h5dcreate_f(file_id, 'bhat_re', H5T_NATIVE_DOUBLE, dspace_coef, dset_coef, hdf_error)
        call h5dwrite_f(dset_coef, H5T_NATIVE_DOUBLE, real(bhat), dims_coef, hdf_error)
        call h5dclose_f(dset_coef, hdf_error)

        call h5screate_simple_f(ndims, dims_coef, dspace_coef, hdf_error)
        call h5dcreate_f(file_id, 'bhat_im', H5T_NATIVE_DOUBLE, dspace_coef, dset_coef2, hdf_error)
        call h5dwrite_f(dset_coef2, H5T_NATIVE_DOUBLE, aimag(bhat), dims_coef, hdf_error)
        call h5dclose_f(dset_coef2, hdf_error)
...
subroutine hdf_read_bhat
    ! Read in continuation file for stochastic wavenumber forcing, bhat coefficients
    use hdf5
    use mpih
    use param
    implicit none
    ...
    ! No need for MPI_BCAST(), as bhat computations are only performed by rank0 in CalcHITRandomForce()
    end if
    end subroutine