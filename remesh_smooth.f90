...
subroutine remesh_smooth(vol_old,target_DV,n_erel,drift,cnt_refresh,nv,ne,nf,xyz,isGhostVert,isGhostEdge,isGhostFace,flagged_edge,&
    ...
    !
    ! Namely, algorithm no. 4 of their work.
    !
    !
    ! We also follow:
    ...
    ! mesh-coarsening algorithm, stored in the flagged_edge array

    omega = 1.0d0 ! Relaxation factor for smoothing, 1.0 for no relaxation
    ...
    drift = 1.0d-18
    min_normA = 99999999.0


    ! Number of iterations to pass through for all edge relaxations
    ! KZ: typically, >1 or >2 is sufficient, most iterations produce sub-Eulerian grid displacements
    !     Smoothing computation is relatively cheap anyway, handful of iterations seems fine
    num_iter = 10
